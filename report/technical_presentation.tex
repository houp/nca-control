...
\begin{block}{AI-Generated Project}
The contents of this project are AI generated.
\end{block}

\begin{block}{Tooling Used}
\begin{itemize}
  \item OpenAI Codex
  \item GPT-5.4 model
  \item High reasoning effort
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
  ...
  \item Python 3.14 as the main interpreter target; Python 3.13 as a secondary re-test path
...
\vspace{0.3em}
\scriptsize
PyTorch also exposes an experimental CUDA path for Linux collaborators. This path is not tested in the current macOS study.
...
Selected minimal model: \texttt{12/3/1}; stronger stability recipe: \(96\) mazes, \(500\) epochs
\end{alertblock}
\end{columns}
\end{frame}
...
\column{0.40\textwidth}
\scriptsize
\begin{block}{Selection Protocol}
\begin{itemize}
...
\begin{itemize}
  \item boundary candidates \(9/3/1\), \(10/3/1\), and \(11/3/1\) all failed
  \item \(12/3/1\): smallest exact and reproducible model found
  \item stronger follow-up: \(96\) mazes, \(500\) epochs, seeds \(0..7\)
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{columns}[T,totalwidth=\textwidth]
\column{0.58\textwidth}
\begin{block}{Stronger Stability Sweep}
\scriptsize
\begin{tabular}{@{}lcc@{}}
\toprule
Evaluation & Setting & Result \\
\midrule
One-step exact & 8 seeds & \(1.0\) \\
Rollout exact & \(30,50\); 8 seeds & \(1.0\) \\
Representative checkpoint A & \(100,200\) & \(1.0\) \\
Representative checkpoint B & \(100,200\) & \(1.0\) \\
\bottomrule
\end{tabular}
\end{block}
...
\small
Exact rollout checks remained valid up to \(200\times 200\).
\end{alertblock}
\end{columns}
\end{frame}

\begin{frame}{Seed Stability Revisit}
\begin{columns}[T,totalwidth=\textwidth]
\column{0.48\textwidth}
\small
\begin{block}{Aggregate Result}
\begin{itemize}
  \item stronger recipe: \(96\) mazes, \(500\) epochs
  \item seeds tested: \(0,1,\dots,7\)
  \item exact pass fraction: \(8/8 = 1.0\)
  \item final loss mean \(\pm\) std: \(0.021730 \pm 0.000664\)
  \item best / worst final loss: \(0.020560\) / \(0.022619\)
\end{itemize}
\end{block}
...
\begin{itemize}
  \item the minimal model now looks robust on the tested seed range
  \item the claim is still empirical, not a proof for all future seeds
  \item large-grid rollout checks remain exact on additional representative checkpoints
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item the current smallest exact architecture is \(12/3/1\), and it was exact on all eight tested retrains under the stronger recipe
\end{itemize}
\end{block}
