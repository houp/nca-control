...
\begin{abstract}
This report summarizes the current state of a research implementation of controllable neural cellular automata (NCA) for a discrete two-dimensional control task. The target environment is a periodic grid containing empty cells, blocked cells, a single controllable active cell, and an exit condition that triggers irreversible terminal dynamics. The implementation combines an exact deterministic reference engine, supervised one-step learning, automated rollout evaluation, a browser-based interactive visualizer, and two learning backends: PyTorch and MLX. The main empirical finding is that the smallest exact model discovered so far remains a \(12/3/1\) architecture, and that under a stronger MLX stability protocol (\(96\) mazes, \(500\) epochs, seeds \(0\) through \(7\)) this model was exact on all eight tested retrains. Representative larger-grid follow-up checks on the formerly suspicious seed \(4\) and the worst-loss seed \(1\) also remained exact on \(100\times 100\) and \(200\times 200\) rollouts.
\end{abstract}
...
The repository retains both a PyTorch implementation and an MLX implementation. PyTorch remains useful for CPU execution and for an experimental CUDA-enabled path intended for collaborators running Linux on NVIDIA hardware, while MLX is currently the preferred Apple Silicon backend. The software environment is now maintained primarily against Python \(3.14\), with Python \(3.13\) retained as a secondary re-test path after major changes. The implementation also includes:
\begin{itemize}
...
The CUDA path was added for portability but was not tested in the present macOS study. Consequently, the verified results reported here should still be interpreted as CPU, MPS, and MLX results collected on Apple Silicon.
...
The minimal-model search itself is based on the strongest width-search protocol used so far:
\begin{itemize}
  \item backend: MLX;
...
After model selection, a dedicated seed-stability follow-up used a stronger fixed recipe:
\begin{itemize}
  \item backend: MLX;
  \item task: \texttt{maze\_exit};
  \item training grid: \(9\times 9\);
  \item training mazes: \(96\);
  \item epochs: \(500\);
  \item batch size: \(128\);
  \item stability seeds: \(0,1,\dots,7\).
\end{itemize}
...
\subsection{Multi-seed stability under a stronger recipe}
An earlier maintenance audit had produced one concerning anecdote: a fresh exploratory seed-\(4\) run under the older \(64\)-maze, \(300\)-epoch recipe plateaued at higher loss. That observation motivated a dedicated follow-up sweep with a stronger MLX recipe using \(96\) mazes, \(500\) epochs, and eight seeds.

Under that stronger protocol, all eight tested seeds were exact on one-step evaluation and on the \(30\times 30\) and \(50\times 50\) rollout checks. Table~\ref{tab:stability} summarizes the aggregate statistics. The exact pass fraction was \(8/8\), the mean final loss was \(0.021730\), and the standard deviation across seeds was only \(0.000664\). In other words, the previously concerning seed-\(4\) failure was not reproduced under the stronger budget.

Figure~\ref{fig:loss} now shows the full set of recorded training-loss curves for seeds \(0\) through \(7\). The curves converge into a narrow low-loss band rather than splitting into clearly distinct success and failure regimes.

\begin{table}[t]
\centering
\caption{Aggregate result of the stronger MLX seed-stability sweep for the selected \(12/3/1\) model.}
\begin{tabular}{lc}
\toprule
Statistic & Value \\
\midrule
Training recipe & \(9\times 9\), \(96\) mazes, \(500\) epochs \\
Seed set & \(0,1,2,3,4,5,6,7\) \\
Exact pass fraction & \(8/8 = 1.0\) \\
Final loss mean \(\pm\) std & \(0.021730 \pm 0.000664\) \\
Final loss median & \(0.021758\) \\
Minimum loss mean \(\pm\) std & \(0.021634 \pm 0.000674\) \\
...
\addplot[thick, blue!80!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed0.csv};
\addlegendentry{seed 0}
\addplot[thick, red!75!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed1.csv};
\addlegendentry{seed 1}
\addplot[thick, teal!80!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed2.csv};
\addlegendentry{seed 2}
\addplot[thick, orange!90!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed3.csv};
\addlegendentry{seed 3}
\addplot[thick, violet!80!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed4.csv};
\addlegendentry{seed 4}
\addplot[thick, green!60!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed5.csv};
\addlegendentry{seed 5}
\addplot[thick, cyan!70!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed6.csv};
\addlegendentry{seed 6}
\addplot[thick, brown!80!black] table[x=epoch,y=loss,col sep=comma] {data/seed_stability_loss_seed7.csv};
\addlegendentry{seed 7}
\end{semilogyaxis}
\end{tikzpicture}
\caption{Training-loss traces for the stronger MLX seed-stability sweep on seeds \(0\) through \(7\). The plotted CSV files are generated directly from the recorded progress artifacts.}
\label{fig:loss}
\end{figure}

\subsection{Generalization to larger grids}
The strongest current evidence for scale robustness is that the selected \(12/3/1\) model remained exact on larger unseen grids in both the earlier validation experiments and the new seed-stability follow-up. In addition to the \(30\times 30\) and \(50\times 50\) rollout checks passed by all eight seeds in the stability sweep, representative larger-grid follow-ups were repeated on seed \(4\) (the formerly suspicious seed) and seed \(1\) (the worst-loss seed in the new sweep).

\begin{table}[t]
\centering
\caption{Representative larger-grid verification after the stronger seed-stability sweep.}
\begin{tabular}{lccc}
\toprule
Evaluation & Grid & Rollout setting & Result \\
\midrule
One-step stability sweep & \(9\times 9\) & 8 clean seeds & exact pass fraction \(=1.0\) \\
Rollout stability sweep & \(30\times 30\) & 8 clean seeds & exact pass fraction \(=1.0\) \\
Rollout stability sweep & \(50\times 50\) & 8 clean seeds & exact pass fraction \(=1.0\) \\
Rollout follow-up, seed \(4\) & \(100\times 100\) & 32 sequences, 200 steps & exact rollout rate \(=1.0\) \\
Rollout follow-up, seed \(4\) & \(200\times 200\) & 16 sequences, 200 steps & exact rollout rate \(=1.0\) \\
Rollout follow-up, seed \(1\) & \(100\times 100\) & 32 sequences, 200 steps & exact rollout rate \(=1.0\) \\
Rollout follow-up, seed \(1\) & \(200\times 200\) & 16 sequences, 200 steps & exact rollout rate \(=1.0\) \\
\bottomrule
\end{tabular}
...
Third, the present model should now be described as \emph{strongly reproducible under the tested stronger recipe}, though still not as universally seed-invariant. The original minimal-model search established exactness on seeds \(0\) through \(3\). The new stability sweep extended that evidence to seeds \(0\) through \(7\) under a larger \(96\)-maze, \(500\)-epoch recipe, and all eight runs passed. This materially weakens the earlier concern raised by the old exploratory seed-\(4\) failure, which appears to have been a weakness of the earlier budget rather than a persistent failure mode of the selected architecture.
...
The main empirical conclusion is that the currently smallest exact model discovered so far remains a \(12/3/1\) architecture trained with MLX. The original minimal-model search selected it under a \(9\times 9\), \(64\)-maze, \(300\)-epoch protocol, and the subsequent stronger stability study showed that the same architecture remains exact on all eight tested retrains under a \(96\)-maze, \(500\)-epoch recipe. Representative follow-up checks on seeds \(1\) and \(4\) also remained exact on \(100\times 100\) and \(200\times 200\) rollouts. The immediate next research question is whether an even more local training formulation, such as patch-based supervision, can preserve these guarantees while further reducing training cost.
